...
3. 按 CLAUDE.md 8-1 的必查项逐项核对（README 同步、测试覆盖、名实一致、不重复造轮子、隐含前提有 guard、禁止魔法值混入输出、输出正交性、Tell Don't Ask）
4. 对于 diff 中涉及的自定义类型，以及审查意见指控有问题的代码所涉及的类型，必须追踪到其实现文件，验证实际行为后再下结论
5. 凡需用搜索（grep 等）确认"某调用不存在/某字段无人使用"的判断（如核对输出正交性、Tell-Don't-Ask、无调用者），适用 CLAUDE.md 2-2：搜索返回空不得直接判定不存在，须换一种方法二次确认后再下结论
...
严格按 CLAUDE.md 8-1 的必查项逐项核对，每项都给出明确结论（通过 / 存在问题）：
① README 同步  ② 测试覆盖（标准见 CLAUDE.md 6-1）  ③ 名实一致  ④ 不重复造轮子
⑤ 隐含前提有 guard  ⑥ 禁止魔法值混入输出  ⑦ 输出正交性  ⑧ Tell, Don't Ask
每条发现注明对应文件和行号（如已知）。全部通过时写"各项均通过"。 -->
