---
name: issue
description: 拉取指定仓库和时间范围内的 GitHub Issue，逐条翻译成中文并保存到 ~/data/issue/<repo>/<number>.md
disable-model-invocation: false
allowed-tools: Bash Read Write
---

# /issue — GitHub Issue 中文归档

## Step 1 — 确认参数

### 模式 A：直接指定编号列表

若用户在调用时直接给出了 issue 编号列表（如 `#1335 #1366` 或 `1335,1366`）以及所属仓库，则：
- **跳过所有其他条件询问**（时间范围、指派人、发布者）
...
- **指派人（assignee）**：筛选被指派给特定用户的 issue，默认**不限制**
- **发布者（author）**：筛选由特定用户创建的 issue，默认**不限制**

如果用户在调用时没有提供上述参数，**用 AskUserQuestion 询问**，提供以下选项：
- 仓库：三个仓库（可多选），含"全部"选项
- 时间范围：最近 7 天 / 最近 30 天 / 自定义（让用户输入）
- 指派人：不限制 / 指定用户名（Other 输入）
- 发布者：不限制 / 指定用户名（Other 输入）

确认后，将仓库列表、起始日期、指派人、发布者固定下来，后续步骤直接使用，不再询问。
...
- 若设置了**发布者**，在 `--search` 字符串中追加 `author:<login>`
- 若设置了**指派人**，追加 `--assignee <login>` 参数
- 若两者均不限制，则不添加对应参数

记录返回的 issue 编号列表。如果列表为空，告知用户该仓库在此条件下没有 issue，跳过。

---

## Step 2.5 — 覆盖确认

在拿到待处理编号列表后，检查其中哪些在 `~/data/issue/<repo>/` 下已存在对应 `.md` 文件：
...
将上述 JSON 内容翻译成中文，写入 `~/data/issue/<repo>/<number>.md`。

**翻译规则**：
1. 所有正文（标题、正文、评论）译为中文；专有名词、技术术语后用括号注释英文原文，例如：拉取请求（Pull Request）、分支（branch）、标签（label）
2. 代码块（`` ``` `` 包裹的内容）**保持原文不翻译**
