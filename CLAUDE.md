...
16. **后台进程监控：用 Monitor 工具 + until 循环，退出条件必须包含进程存活检查**：等待后台进程完成时，使用 Monitor 工具执行 until 循环（Claude Code 官方规范），不得在 Bash 工具内用 `sleep` 轮询。退出条件必须同时覆盖"成功完成"和"进程已死"两种情况，不能只依赖 log 内容匹配——进程被 OOM kill、信号终止时不会写入 log，单靠 log 匹配会永远不退出。标准模板：`until [ -f <完成标志文件> ] || ! kill -0 <PID> 2>/dev/null; do sleep 5; done`。
...
18. **接口修改时主动提醒更新测试用例**：代码修改涉及公开接口变更（函数签名、参数名、返回值结构、事件格式等）时，必须主动提醒用户检查并更新对应的测试用例，不得默认测试用例会自动跟进。
19. **实验产物结构变更时主动核对 README**：代码修改涉及实验输出目录结构、文件名或文件含义变化时，必须主动与 `app2/README.md` 的输出结构描述比对，询问用户是否立即更新 README；若用户选择暂不更新，在该分支执行 `git push` 前必须再次询问一次。
20. **解释代码时的方法论**：
