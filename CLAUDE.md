...
**9-5. 本地 issue/PR markdown 文件只能通过 `/issue`、`/pr` skill 修改**：`~/data/issue/` 和 `~/data/pr/` 下的所有文件由这两个 skill 统一管理，保证与远程内容同步。除 Read 之外，禁止对这些文件执行任何写操作（Edit、Write、删除等），也禁止用 Bash 写入。需要更新时，通过 `/issue` 或 `/pr` skill 重新拉取。
